...
fn associative<T: Eq + Copy>(f: fn(T, T) -> T) -> fn(T, T, T) -> bool {
    Box::new(move |a, b, c| f(a, f(b, c)) == f(f(a, b), c))
}
...
fn main() {
    ...
    match QuickCheck::new().quicktest(associativity as fn(i32, i32, i32) -> bool) {
        Ok(n) => println!("Probably associative ({} tests passed)", n),
        Err(e) => println!("Not associative {:?}", e),
    }
    match QuickCheck::new().quicktest(commutativity as fn(i32, i32) -> bool) {
        Ok(n) => println!("Probably commutative ({} tests passed)", n),
